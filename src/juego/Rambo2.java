...
public class Rambo2 {
	...
	public Rambo2(double x, double y, int ancho, int alto, double f, Image imagenDerecha, Image imagenIzquierda,Image imagenArriba, Image imagenAbajo, int anchoPantalla, int altoPantalla) {
		this.x = x;
		this.y = y;
		this.ancho = ancho;
		this.alto = alto;
	...
	public void moverIzquierda(Entorno e) {
		...
		if (e.estaPresionada(e.TECLA_IZQUIERDA) && !limitesMapaIzquierda() && BloqA==false) {
	...
	public void moverDerecha(Entorno e) {
		...
		if (e.estaPresionada(e.TECLA_DERECHA) && !limitesMapaDerecha() && BloqD==false) {
			//verifica límites del mapa y limitan el movimiento
			BloqW=true; //Bloqueo de teclas
			BloqS=true;
			x += factorDesplazamiento;
	...
	public void moverArriba(Entorno e) {
		...
		if (e.estaPresionada(e.TECLA_ARRIBA) && !limitesMapaArriba() && BloqW==false) {
			//verifica límites del mapa y limitan el movimiento
	...
	public void moverAbajo(Entorno e) {
		...
		if (e.estaPresionada(e.TECLA_ABAJO) && !limitesMapaAbajo() && BloqS==false) {
			//verifica límites del mapa y limitan el movimiento
			BloqA=true; //Bloqueo de teclas
			BloqD=true;
			y += factorDesplazamiento;
	...
	public void disparar(Entorno e, List<LaserAzul> lasers){
		if (e.sePresiono(e.TECLA_ENTER) && Juego.bloqueoDisparo2==false) {
			if (sonidoDisparo != null) {
				sonidoDisparo.setFramePosition(0); // Reinicia la reproducción al principio
				sonidoDisparo.start();
			}
			Juego.bloqueoDisparo2=true;
			...
			Image imagenDisparo = Herramientas.cargarImagen("laserBlueIzq.png"); // Imagen por defecto Izquierda
			...
			if (teclaArribaPresionada) {
				...
				imagenDisparo = Herramientas.cargarImagen("laserBlueArr.png");
				hitboxLaser.setAncho(8);
				hitboxLaser.setAlto(86);
			} else if (teclaAbajoPresionada) {
				direccionDisparo = Math.PI / 2; // Disparo hacia abajo
				imagenDisparo = Herramientas.cargarImagen("laserBlueAbj.png");
				hitboxLaser.setAncho(8);
				hitboxLaser.setAlto(86);
			} else if (teclaIzquierdaPresionada) {
				direccionDisparo = Math.PI; // Disparo hacia la izquierda
				imagenDisparo = Herramientas.cargarImagen("laserBlueIzq.png");
				hitboxLaser.setAncho(86);
				hitboxLaser.setAlto(8);
			} else if (teclaDerechaPresionada) {
				direccionDisparo = 0; // Disparo hacia la derecha
				imagenDisparo = Herramientas.cargarImagen("laserBlueDer.png");
				hitboxLaser.setAncho(86);
				hitboxLaser.setAlto(8);
			}

			LaserAzul laser = new LaserAzul(centroX, centroY, 1, 5, direccionDisparo, 25, 0, imagenDisparo, hitboxLaser);
			lasers.add(laser);	            	        	            	            
		}
	}
}
